...
Je code zal een `for` lus gebruiken om 28 keer te werken. **Each** keer zal het:
...
**Find** je `rgb = sense.color` codelijn.

**Add** code hierboven om je `for` lus voor `28` herhalingen in te stellen.
...
Je zal nu alle code onder de `for` lus moeten opslaan zodat die bewaard wordt **inside** de `for` lus.
...
**Tip:** Zorg ervoor dat je **do not** de codelijn `sense.clear()` opslaat want je wil dit maar 1 keer laten gebeuren op het einde van je animatie.
